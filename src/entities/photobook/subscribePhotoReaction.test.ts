...
describe('subscribePhotoReaction', () => {
  beforeEach(() => {
    vi.clearAllMocks()
  })
  ...
  it('calls onChange with true when the reaction doc exists', () => {
    ...
    const result = subscribePhotoReaction('photo-1', 'uid-1', onChange)
  ...
  it('calls onChange with false when the reaction doc does not exist', () => {
    ...
    const onChange = vi.fn()
    subscribePhotoReaction('photo-1', 'uid-1', onChange)

    expect(onChange).toHaveBeenCalledWith(false)
  })
})

describe('SubscribePhotoReactionToken', () => {
  it('carries a description identifying the use case', () => {
    expect(SubscribePhotoReactionToken.description).toBe(
      'SubscribePhotoReaction'
    )
  })
})
